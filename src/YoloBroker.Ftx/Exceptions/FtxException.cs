...
namespace YoloBroker.Ftx.Exceptions
{
    public class FtxException : BrokerException
    {
        ...
        public FtxException(string message, IEnumerable<(Trade trade, WebCallResult<FTXOrder> result)> failedResults)
            : base($"{message}:{NewLine}{NewLine}{string.Join(NewLine, failedResults.Select(FormatErrorInfo))}")
        {
        }
