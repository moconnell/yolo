...
{
    public static class FTXExtensions
    {
        ...
        public static DateTime? GetExpiry(this FTXSymbol symbol)
        {
            ...
            {
                _ when FutureMonthDayExpiryRegex.IsMatch(symbol.Name) => GetDayMonthExpiry(symbol.Name),
                _ when FutureQuarterlyExpiryRegex.IsMatch(symbol.Name) => GetQuarterlyExpiry(symbol.Name),
                _ => null
            };
        }
